...
def calculate_live_projection(h_live_score, a_live_score, period, clock_str, pre_h_score, pre_a_score, h_modifier=0.0, a_modifier=0.0):
    """Calculates the live projected final score with PLAYOFF MATH and EXPONENTIAL DECAY applied."""
    try:
    ...
    # ==========================================================
    # THE FIX: EXPONENTIAL TIME DECAY
    # ==========================================================
    # Squaring the time fraction suppresses early-game variance.
    # Q1 (25% done) -> 0.25^2 = 6.25% live weight (Trusts pre-game 93.75%)
    # Halftime (50% done) -> 0.50^2 = 25.0% live weight (Trusts pre-game 75%)
    # End of Q3 (75% done) -> 0.75^2 = 56.2% live weight
    time_fraction = seconds_elapsed / total_seconds
    live_weight = time_fraction ** 2.0
    ...
    # ==========================================================
    # --- PLAYOFF IN-GAME MATH TWEAKS ---
    # ==========================================================
    # 1. PLAYOFF PACE: Games grind to a halt in the 2nd half
    PLAYOFF_PACE_MODIFIER = 0.965
    h_expected_remaining *= PLAYOFF_PACE_MODIFIER
    a_expected_remaining *= PLAYOFF_PACE_MODIFIER

    # 2. HOME WHISTLE: Amplified home momentum in playoffs (+2.5 pts)
    h_expected_remaining += 2.5
    
    # 3. BLOWOUT / GARBAGE TIME DAMPENER
    # If a team is up by 15+ in the 2nd half, both teams milk the clock
    if period >= 3:
        score_diff = abs(h_live_score - a_live_score)
        if score_diff >= 15:
            h_expected_remaining *= 0.90
            a_expected_remaining *= 0.90
    # ==========================================================
...
def run_live_loop():
    print("Starting Live NBA Playoff Tracker with Quota Protection...")
