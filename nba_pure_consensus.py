...
def adjust_total_for_modern_era(predicted_total, home, away, mc_stats, vol_3p):
    ...
    pace_derived_total = avg_pace * 1.18 * 2
...
def update_final_scores():
    ...
    try:
        ...
        for game in res:
            ...
            # --- NEW: Convert UTC to Eastern to match the inserted game_id ---
...
def run_consensus():
    ...
    for m in matchups:
        ...
        rest_home = calculate_days_rest(home, current_date, gamelog)
        rest_away = calculate_days_rest(away, current_date, gamelog)
...
# ---------------------------------------------------------
        ...
        if home in mc_stats.index and away in mc_stats.index:
            ...
            ortg_diff = (mc_stats.loc[home]['avg_ortg'] + 2.3) - mc_stats.loc[away]['avg_ortg']
            
            efg_impact = row['efg_mismatch'].values[0] * 200 
            tov_impact = row['tov_mismatch'].values[0] * 125
